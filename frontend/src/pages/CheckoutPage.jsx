...
export default function CheckoutPage() {
  ...
  // never qualifies server-side.
  const freeShippingApplies =
    freeShippingThreshold > 0 &&
    subtotal >= freeShippingThreshold &&
    paymentMethod !== 'cod';
  const shippingAmount = freeShippingApplies ? 0 : quotedShipping;
  ...
  function startPay() {
    ...
    // customer_phone is optional server-side, but when sent it must be ≥8
    // chars (CheckoutRequest min_length) — validate for every payment method
    // so the backend never rejects with a generic 422.
    ...
    try {
      const resp = await checkout.mutateAsync({
        ...
        ...(instrumentApplies && paymentInstrument
          ...
          : {}),
        // Only send phone when it satisfies the backend's min_length=8.
